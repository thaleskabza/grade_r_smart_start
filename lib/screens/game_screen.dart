...
class _GameScreenState extends State<GameScreen> {
  ...
  void _generateSequenceGame() {
    ...
    _sequenceOptions = [_missingValue!, _missingValue! + 1, _missingValue! - 1]..shuffle();
  }
  ...
  void _generateClockGame() {
    ...
    _timeOptions = [
      ...
      TimeOfDay(hour: (hour + 2) % 12 + 1, minute: (minute + 15) % 60).format(context)
    ]..shuffle();
  }
 ...
 void _checkAnswer(dynamic answer) async {
  if (isCorrect != null) {
    print("Answer already checked. Ignoring new input.");
    return;
  }
  ...
  final String childName = widget.profile.name.isNotEmpty ? widget.profile.name : "Learner";
  ...
  try {
    ...
    print("Score saved for $childName | Correct: $correct");
  } catch (e) {
  ...
  if (correct) {
    print("TTS: Speaking congratulations");
    await _flutterTts.speak("Congratulations! You got it right!");
  } else {
    print("TTS: Speaking encouragement");
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            ...
            if (_selectedGame == GameType.number) ...[
              Text('Tap the number $targetNumber', style: const TextStyle(fontSize: 22)),
              ...
              Wrap(
                ...
                children: List.generate(maxRange, (i) {
                  ...
                  return ElevatedButton(
                    onPressed: isCorrect == null ? () => _checkAnswer(val) : null,
                    child: Text('$val'),
                  );
                }),
              ),
            ] else if (_selectedGame == GameType.day) ...[
              Text('What day comes after $currentDay?', style: const TextStyle(fontSize: 22)),
              const SizedBox(height: 10),
              Wrap(
                spacing: 10,
                children: _days.map((day) {
                  return ElevatedButton(
                    onPressed: isCorrect == null ? () => _checkAnswer(day) : null,
                    child: Text(day),
                  );
                }).toList(),
              ),
            ] else if (_selectedGame == GameType.sequence) ...[
              const Text('Fill in the missing number:', style: TextStyle(fontSize: 22)),
              const SizedBox(height: 10),
              Row(
              ...
              Wrap(
                spacing: 10,
                children: _sequenceOptions.map((opt) {
                  return ElevatedButton(
                    onPressed: isCorrect == null ? () => _checkAnswer(opt) : null,
                    child: Text('$opt'),
                  );
                }).toList(),
              ),
              ...
              const SizedBox(height: 10),
              const Icon(Icons.access_time_filled, size: 80, color: Colors.deepPurple),
              const SizedBox(height: 10),
              Wrap(
                spacing: 10,
